...
import { TimelineElement } from '../../Data/models/events-timeline.model';
import {
  animate,
  keyframes,
  state,
...
export class HorizontalTimelineComponent implements AfterViewInit {
  ...
  @ViewChild('eventsWrapper') eventsWrapper: ElementRef | undefined;
  @ViewChild('fillingLine') fillingLine: ElementRef | undefined;
  @ViewChildren('timelineEvents') timelineEvents:
  ...
  private static minLapse(elements: TimelineElement[]): number {
    ...
    for (let i = 1; i < elements.length; i++) {
      let distance = HorizontalTimelineComponent.dayDiff(
        elements[i - 1].date,
        elements[i].date
      );
  ...
  updateSlide(timelineTotWidth: number, forward: boolean) {
    ...
    let translateValue = HorizontalTimelineComponent.getTranslateValue(
      this.eventsWrapper.nativeElement
    );

    if (forward) {
      this.translateTimeline(
        translateValue - this._timelineWrapperWidth + this._eventsMinDistance,
        this._timelineWrapperWidth - timelineTotWidth
      );
    } else {
      this.translateTimeline(
        translateValue + this._timelineWrapperWidth - this._eventsMinDistance,
  ...
  updateTimelinePosition(element: Element) {
    ...
    let eventLeft = HorizontalTimelineComponent.pxToNumber(
      eventStyle.getPropertyValue('left')
    );
    if (!this.eventsWrapper) {
      return;
    }
    ...
    if (eventLeft > this._timelineWrapperWidth - translateValue) {
      this.translateTimeline(
        -eventLeft + this._timelineWrapperWidth / 2,
        this._timelineWrapperWidth - this.eventsWrapperWidth
      );
    }
  }
  ...
  translateTimeline(value: number, totWidth: number | null) {
    ...
    HorizontalTimelineComponent.setTransformValue(
      this.eventsWrapper.nativeElement,
      'translateX',
      value + 'px'
    );
  ...
  setTimelineWidth(
    ...
    if (elements.length > 1) {
      timeSpan = HorizontalTimelineComponent.dayDiff(
        elements[0].date,
    ...
    let aHref =
      this.eventsWrapper.nativeElement.querySelectorAll('a.selected')[0];
  ...
  private updateFilling(element: any) {
    ...
    let eventLeftNum =
      HorizontalTimelineComponent.pxToNumber(eventLeft) +
      HorizontalTimelineComponent.pxToNumber(eventWidth) / 2;
  ...
  private setDatePosition(
    ...
    for (let component of elements) {
      ...
      timelineEventsArray[i].nativeElement.style.left =
        distanceNorm * min + 'px';
      // span
      let span: HTMLSpanElement = <HTMLSpanElement>(
        timelineEventsArray[i].nativeElement.parentElement.children[1]
      );
